...
var debugx = require('../utils/pinbug.js')('devebot:securityManager');
...
var Service = function(params) {
  ...
  self.authenticate = function(tokens) {
    ...
    debugx.enabled && debugx(' - authenticate(%s): %s', JSON.stringify(tokens),
  ...
  var loadTokenStore = function(storefile) {
    ...
    return readFile(storefile, 'utf8').then(function(text) {
      ...
      if (lodash.isEmpty(data.tokens) || !lodash.isArray(data.tokens)) {
        debugx.enabled && debugx(' - invalid tokenStore (%s), "tokens" must be an array', storefile);
        return {};
      }
      debugx.enabled && debugx(' - tokenStore (%s) has %s items', storefile, data.tokens.length);
      return data;
    }).catch(function(err) {
      debugx.enabled && debugx(' - tokenStore loading has failed. Error: %s', JSON.stringify(err));
      return {};
    });
  };
