...
var debugx = require('../utils/pinbug.js')('devebot:bridgeLoader');
...
var buildBridgeDialect = function(bridgeCode, bridgeRecord, dialectName, optType) {
  ...
  debugx.enabled && debugx(' - build bridgeDialect (%s) is started', dialectName);
  ...
  function dialectConstructor(params) {
    params = params || {};
    ...
    var newFeatures = lodash.get(params, ['profileConfig', 'newFeatures', dialectName], null);
    if (newFeatures === null) {
      newFeatures = lodash.get(params, ['profileConfig', 'newFeatures', bridgeCode], {});
    }
    debugx.enabled && debugx(' - newFeatures[%s]: %s', dialectName, JSON.stringify(newFeatures));

    if (newFeatures.logoliteEnabled) {
      var loggingFactory = params.loggingFactory.branch(uniqueName);
      this.logger = loggingFactory.getLogger();
      this.tracer = loggingFactory.getTracer();
    } else {
      this.logger = params.loggingFactory.getLogger({ scope: uniqueName });
    }
  ...
  debugx.enabled && debugx(' - build bridgeDialect (%s) has done.', dialectName);
...
var buildBridgeDialects = function(bridgeRefs, dialectOptions, optType) {
  ...
  debugx.enabled && debugx(' - bridgeDialects will be built: %s', JSON.stringify(bridgeRefs));
  ...
  debugx.enabled && debugx(' - bridgeDialects have been built: %s', JSON.stringify(lodash.keys(bridgeDialects)));
