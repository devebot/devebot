...
var buildBridgeWrapper = function(bridgeCode, bridgeConstructor, wrapperName, optType) {
  ...
  function wrapperConstructor(params) {
    ...
    var legacyMode = true;
    if (!lodash.isEmpty(newFeatures)) {
      if (newFeatures.sandboxConfig) {
        params = getWrappedParams();
        params.sandboxConfig = lodash.get(params, configPath, {});
        legacyMode = false;
      }
      ...
      if (newFeatures.logoliteEnabled) {
        ...
        newFactory.getTracer = function() {
          return LogTracer.ROOT.branch({ key: 'bridgeName', value: wrapperName });
        }
      ...
      if (legacyMode === false) {
        delete params.profileConfig;
        bridgeConstructor.call(this, params);
      }
    }
