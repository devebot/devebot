...
var Service = function(params) {
  ...
  var sandboxList = {
    singleton: params.sandboxConfig
  };
  var sandboxes = {};
  var dialectRefs = {};

  lodash.forOwn(sandboxList, function(value, key) {
    var sandbox = {};

    LX.has('conlog') && LX.log('conlog', LT.add({
      sandboxName: key,
      sandboxCfg: util.inspect(value),
    }).stringify({
      text: ' - load the sandbox[{sandboxName}] with configuration: {sandboxCfg}'
    }));

    LX.has('conlog') && LX.log('conlog', LT.add({
      sandboxName: key
    }).stringify({
      text: ' - create sandbox[{sandboxName}].injektor object',
      reset: true
    }));
  ...
  self.eachTriggers = function(iteratee, sandboxNames, triggerNames, actionName) {
    ...
    LX.has('conlog') && LX.log('conlog', ' - Loop sandboxes: %s', JSON.stringify(sandboxNames || 'all'));
      ...
      if (!sandboxNames || sandboxNames.indexOf(sandboxName) >= 0) {
        lodash.forOwn(triggerMap, function(triggerRecord, triggerId) {
          ...
          if (!triggerNames || triggerNames.indexOf(triggerName) >= 0) {
            LX.has('conlog') && LX.log('conlog', ' - %s sandbox[%s][%s]', actionName, sandboxName, triggerName);
