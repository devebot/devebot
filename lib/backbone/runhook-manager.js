...
var Service = function(params) {
  ...
  var getRunhook = function(runinfo) {
    ...
    var fn = runhookStore.fullnameOf(runinfo.name);
  ...
  self.execute = function(runinfo, context) {
    ...
    if (runinfo.mode !== 'remote' || params.jobqueueMaster.enabled === false) {
      ...
      return self.process(runinfo, progressMeter).then(function(result) {
      ...
      return params.jobqueueMaster.enqueueJob(runinfo, context);
    }
  };
  ...
  self.process = function(runinfo, progressMeter) {
    ...
    if (lodash.isFunction(handler)) {
      if (predefinedContext) {
        ...
        return Promise.resolve(handler.call(buildRunhookInstance(runinfo.name), payload, context));
      }
    } else {
