...
var Service = function(params) {
  ...
  var runhookMap = {};
  var runhookStore = new Injektor();

  var getRunhooks = function() {
    return (runhookMap[constx.RUNHOOK.ROOT_KEY] = runhookMap[constx.RUNHOOK.ROOT_KEY] || {});
  };

  var getRunhook = function(runinfo) {
    if (!runinfo || !runinfo.name) return {};
    var fn = runhookStore.suggest(runinfo.name);
    if (fn == null || fn.length == 0 || fn.length >= 2) return {};
    return runhookStore.lookup(runinfo.name, {
    ...
    if (runinfo.mode !== 'remote' || params.jobqueueMaster.enabled === false) {
      ...
      promize = params.jobqueueMaster.instance.enqueueJob(runinfo).then(function(task) {
        return new Promise(function(onResolved, onRejected) {
          task
            .on('started', function(data) {
              context.outlet && context.outlet.render('started', data);
            })
            .on('progress', function(data) {
              context.outlet && context.outlet.render('progress', data);
            })
            ...
            .on('completed', function(result) {
              context.outlet && context.outlet.render('completed', { result: result });
              onResolved(result);
            })
            .on('failed', function(error) {
              context.outlet && context.outlet.render('failed', { error: error });
              onRejected(error);
            });
        });
      });
  ...
  self.process = function(runinfo, context) {
    ...
    if (lodash.isFunction(handler)) {
      if (predefinedContext) {
        return Promise.resolve(handler(options, payload, context));
      } else {
        return Promise.resolve(handler.call(buildRunhookInstance(runinfo.name), options, payload, context));
      }
    } else {
  ...
  lodash.forOwn(getRunhooks(), function(value, key) {
    runhookStore.registerObject(value.name, value.object, { scope: value.moduleId });
  });
