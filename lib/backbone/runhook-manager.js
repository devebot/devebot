...
var Service = function(params) {
  ...
  self.execute = function(runhook, context) {
    ...
    var validator = runhooks[runhook.name] && runhooks[runhook.name].validator;
    if (validator && lodash.isFunction(validator)) {
      debugx.enabled && debugx('validate runhook: %s using validator function');
      if (!validator(payload)) {
        validationError = {
          message: 'failed validation using validator function'
        };
      }
    }
