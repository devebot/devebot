...
var Service = function(params) {
  ...
  self.execute = function(command, context) {
    var runhook = getRunhook(command);
    LX.has('trace') && LX.log('trace', LT.add({
      sandboxName: getSandboxName(),
      runhook: command
    }).stringify({
      text: 'runhookManager on sandbox<{sandboxName}> - runhook: <{command}> - validate'
    }));
    ...
    LX.has('trace') && LX.log('trace', 'runhookManager on sandbox[%s] - runhook: %s - enqueue',
      getSandboxName(), JSON.stringify(command));

    var promize = null;
    if (command.mode !== 'remote' || params.jobqueueMaster.enabled === false) {
      ...
      promize = params.jobqueueMaster.instance.enqueueJob(command).then(function(task) {
  ...
  self.process = function(command, context) {
    ...
    LX.has('trace') && LX.log('trace', 'runhookManager on sandbox[%s] - command: %s - process',
      getSandboxName(), JSON.stringify(command));
