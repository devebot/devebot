...
var Service = function(params) {
  ...
  self.execute = function(runinfo, context) {
    ...
    if (runinfo.mode !== 'remote' || params.jobqueueMaster.enabled === false) {
      ...
      promize = self.process(runinfo, progressMeter).then(function(result) {
      ...
      promize = params.jobqueueMaster.enqueueJob(runinfo, context);
    }
    return promize;
  };

  self.process = function(runinfo, progressMeter) {
    runinfo = runinfo || {};
