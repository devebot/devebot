...
var debugx = debug('devebot:jobqueue:worker');
...
var Service = function(params) {
  ...
  var jobQueueProcess = function (job, done) {
    ...
    if (runhookManager.isRunhookAvailable(runhook)) {
      if (self.logger.isLevelEnabled('trace')) {
        self.logger.trace('jobqueueWorker on sandbox[%s] - call runhook: %s',
          self.getSandboxName(), JSON.stringify(runhook_log));
      }
      runhookManager.callRunhook(runhook).then(function(result) {
        if (self.logger.isLevelEnabled('trace')) {
