...
var debugx = require('../utils/debug.js')('devebot:pluginLoader');
...
function PluginLoader(params) {
  ...
  var loaderClass = {
    processMonitor: params.processMonitor,
...
var loadScriptEntry = function(scriptMap, scriptType, scriptSubDir, scriptFile, scriptContext, pluginRootDir) {
  ...
  try {
    ...
    if (lodash.isFunction(scriptInit)) {
      ...
      if (!output.valid) {
        debugx.enabled && debugx(' - script validation is failed: %s', JSON.stringify(output));
        opStatus.hasError = true;
      } else {
        debugx.enabled && debugx(' - script validation has been passed');
    ...
    debugx.enabled && debugx(' - script file %s loading has failed.', filepath);
    opStatus.hasError = true;
...
var loadGadgetEntry = function(gadgetMap, gadgetType, gadgetSubDir, gadgetFile, pluginRootDir) {
  ...
  try {
    ...
    debugx.enabled && debugx(' - gadget file %s loading has done', filepath);
    if (lodash.isFunction(gadgetConstructor)) {
      ...
      debugx.enabled && debugx(' - gadget file %s doesnot contain a function', filepath);
      opStatus.hasError = true;
    }
  } catch(err) {
    debugx.enabled && debugx(' - gadget file %s loading has failed', filepath);
    opStatus.hasError = true;
    opStatus.stack = err.stack;
  }
  self.processMonitor.collect(opStatus);
...
var buildGadgetWrapper = function(gadgetConstructor, wrapperName, pluginRootDir) {
  ...
  var pluginName = chores.stringCamelCase(pluginRootDir.name);
  ...
  function wrapperConstructor(params) {
    ...
    debugx.enabled && debugx(' - newFeatures[%s]: %s', pluginName, JSON.stringify(newFeatures));
    ...
    if (newFeatures.logoliteEnabled) {
      params = getWrappedParams();
      var newFactory = lodash.clone(params.loggingFactory || {});
      newFactory.getLogger = params.loggingFactory.getWrappedLogger;
      var serviceTracer = null;
      newFactory.getTracer = function() {
        if (serviceTracer === null) {
          serviceTracer = LogTracer.ROOT
            .branch({ key: 'serviceName', value: wrapperName })
            .branch({ key: 'serviceId', value: LogTracer.getLogID() });
          var serviceLogger = newFactory.getLogger();
          serviceLogger.isEnabledFor('info') && serviceLogger.log('info', serviceTracer.add({
            message: 'create new plugin service',
            pluginName: pluginRootDir.name
          }).toMessage({reset:true}));
        }
        return serviceTracer;
      }
      newFactory.getTracer();
      params.loggingFactory = newFactory;
    }
  ...
  debugx.enabled && debugx(' - argumentSchema: %s', JSON.stringify(wrapperConstructor.argumentSchema));

  result[[pluginRootDir.name, wrapperName].join(chores.getSeparator())] = {
    pluginId: pluginRootDir.name,
    name: wrapperName,
