...
function PluginLoader(params) {
  ...
  this.loadCommands = function(commandMap, commandContext) {
    return loadAllScripts(commandMap, 'COMMAND', params.pluginRootDirs, commandContext);
  };

  this.loadRunhooks = function(runhookMap, runhookContext) {
    return loadAllScripts(runhookMap, 'RUNHOOK', params.pluginRootDirs, runhookContext);
  };

  this.loadServices = function(serviceMap) {
    return loadAllGadgets(serviceMap, 'SERVICE', params.pluginRootDirs);
  };

  this.loadTriggers = function(triggerMap) {
    return loadAllGadgets(triggerMap, 'TRIGGER', params.pluginRootDirs);
  };
