...
function PluginLoader(params) {
  ...
  debugx.enabled && debugx(' - pluginLoader start with pluginRootDirs: %s', JSON.stringify(pluginRootDirs, null, 2));

  var loaderClass = { contextMonitor: params.contextMonitor };

  this.loadCommands = function(commandMap, commandContext) {
    return loadAllScripts.call(loaderClass, commandMap, 'COMMAND', pluginRootDirs, commandContext);
  };
