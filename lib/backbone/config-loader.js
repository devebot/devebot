...
function Loader(appName, appRootDir, libRootDirs) {
  ...
  var loadConfig = function(appName, appRootDir, libRootDirs, profileName, sandboxName, customDir, customEnv, specificSandbox) {
    ...
    var privateSandbox = null;
    if (specificSandbox) {
      var configFile = path.join(configDir, 'sandbox_' + specificSandbox + '.js');
      LX.has('conlog') && LX.log('conlog', ' - load the private sandbox config: %s', configFile);
      privateSandbox = loadConfigFile(configFile);
    }

    if (privateSandbox) {
      config[CONFIG_SANDBOX_NAME]['staging'] = lodash.defaultsDeep(privateSandbox,
          config[CONFIG_SANDBOX_NAME]['staging']);
    }
