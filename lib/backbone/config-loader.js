...
function init(appName, appRootDir, libRootDirs) {
  ...
  debugx.enabled && debugx(' + Config of application (%s) is loaded in context: %s', appName, label);
...
init.loadConfig = function(appRootDir, libRootDirs) {
  ...
  configNames.forEach(function(configName) {
    ...
    if (debugx.enabled) debugx(' + load the default config: %s', defaultFile);
    ...
    if (debugx.enabled) debugx(' + load the custom config of %s', configName);
    ...
    lodash.forEach(contextNames, function(contextItem) {
      ...
      if (debugx.enabled) debugx(' - load the environment config: %s', contextFile);
