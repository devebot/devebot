...
var RepeatedTimer = function(kwargs) {
  ...
  var timerTrail = LogTracer.ROOT
      .branch({ key: 'serviceName', value: 'repeatedTimer' })
      .branch({ key: 'serviceId', value: kwargs.timerId || LogTracer.getLogID() });

  LX.has('conlog') && LX.log('conlog', timerTrail.add({
    checkpoint: 'RepeatedTimer.new()'
  }).stringify({
  ...
  this.start = function() {
    LX.has('trace') && LX.log('trace', timerTrail.add({
  ...
  LX.has('conlog') && LX.log('conlog', timerTrail.add({
    checkpoint: 'RepeatedTimer.new() end!'
  }).toMessage({
