...
var Service = function(params) {
  ...
  self.execute = function(cmddef, context) {
    context = context || {};
    ...
    var p = Promise.resolve(cmdobj.handler.call(commandInstance, cmddef.options, context));
    p.then(function(result) {
      if (!result._alreadyWrittenToOutlet) {
        debugx.enabled && debugx('Command [%s] has been completed', cmddef.command);
        context.outlet.render('complete', { result: result });
      }
    }).catch(function(error) {
      if (!error._alreadyWrittenToOutlet) {
        debugx.enabled && debugx('Command [%s] has been failed', cmddef.command);
        context.outlet.render('failed', { error: error });
      }
    }).finally(function() {
      debugx.enabled && debugx('Command [%s] has been done', cmddef.command);
