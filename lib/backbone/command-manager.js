...
var Service = function(params) {
  ...
  var validateCommand = function(target) {
    target = target || {};
    var results = [];

    var targetProps = lodash.pick(target, lodash.keys(constx.COMMAND.SCHEMA.OBJECT.properties));
    results.push(params.schemaValidator.validate(targetProps, constx.COMMAND.SCHEMA.OBJECT));
  ...
  lodash.forOwn(self.getCommands(), function(command, name) {
    var output = validateCommand(command);
    if (!output.valid) {
      result[name] = output;
    }
  });
