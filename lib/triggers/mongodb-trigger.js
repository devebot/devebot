...
var Service = function(params) {
  ...
  var contextname = params.contextname;
  ...
  oplog.on('end', function () {
    logger.trace(' * [%s] - MongoDB oplog stream ended', contextname);
  });
  
  oplog.on('error', function (error) {
    logger.error(' * [%s] - MongoDB oplog has error: %s', contextname, JSON.stringify(error));
  });
  ...
  runhookEntities.forEach(function(entity) {
    ...
    runhookOperations.forEach(function(operation) {
      opfilter.on(operation, (function(jobqueueAdapter, entity, operation) {
        return function(doc) {
          logger.trace(' + [%s] - oplog [%s] event is raised: %s', contextname, operation, mongo_url);
