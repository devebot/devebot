  ...
  controller.executeCommand = function(command, socket) {
    ...
    switch(cmd.command) {
      case constx.CMDSET.SANDBOX_INFO:
        var output = {};
        var mongo_conf = lodash.clone(mongodbHelper.mongo_conf);
        lodash.assign(mongo_conf, { password: '***' });
        Promise.resolve().then(function() {
          lodash.assign(output, {
            state: constx.WEBSOCKET.STATE.SUCCESS,
            type: 'result',
            message: 'Sandbox Information',
            details: [
              {
                type: 'record',
                ...
                label: {
                  ...
                  es_index_url: 'Elasticsearch Index URL',
                  mongodb_url: 'Mongo URL',
                  mongodb_cols: 'Mongo collections'
                },
                data: {
                  sandbox_pointer: sandboxManager.getSandboxPointer(),
                  es_index_url: elasticsearchHelper.es_index_url,
                  es_index_name: elasticsearchHelper.es_index_name,
                  mongodb_url: chores.buildMongodbUrl(mongo_conf),
                  mongodb_cols: JSON.stringify(mongodbHelper.mongo_cols, null, 2)
                }
              }
            ]
          });
      ...
      case constx.CMDSET.ES_INFO:
        ...
        Promise.resolve().then(function() {
          lodash.assign(output, {
            state: constx.WEBSOCKET.STATE.SUCCESS,
            type: 'result',
            message: 'Elasticsearch Information',
            details: [
              {
                type: 'record',
                title: 'Connection information',
                ...
                data: {
                  ...
                  url: elasticsearchHelper.es_url,
                  index_name: elasticsearchHelper.es_index_name,
          ...
          var es_types = lodash.get(mappings, [elasticsearchHelper.es_index_name, 'mappings'], {});
