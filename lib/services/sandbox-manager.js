...
var Service = function(params) {
  ...
  self.startTriggers = function(sandboxNames) {
    if (lodash.isString(sandboxNames)) sandboxNames = [sandboxNames];
    if (sandboxNames && !lodash.isArray(sandboxNames)) return;
    
    logger.trace(' + start sandboxes: %s', JSON.stringify(sandboxNames || 'all'));
    lodash.forOwn(sandboxes, function(value, key) {
      if (!sandboxNames || sandboxNames.indexOf(key) >= 0) {
        logger.trace(' - start sandbox[%s].mongodbTrigger', key);
        sandboxes[key].injektor.lookup('mongodbTrigger').start();
      }
    });
  };
  
  self.stopTriggers = function(sandboxNames) {
    if (lodash.isString(sandboxNames)) sandboxNames = [sandboxNames];
    if (sandboxNames && !lodash.isArray(sandboxNames)) return;
    
    logger.trace(' + stop sandboxes: %s', JSON.stringify(sandboxNames || 'all'));
    lodash.forOwn(sandboxes, function(value, key) {
      if (!sandboxNames || sandboxNames.indexOf(key) >= 0) {
        logger.trace(' - stop sandbox[%s].mongodbTrigger', key);
        sandboxes[key].injektor.lookup('mongodbTrigger').stop();
      }
    });
  };
};
