...
var Service = function(params) {
  ...
  var jobQueueProcess = function (job, done) {
    ...
    if (runhookManager.isRunhookAvailable(routine, entity, operation)) {
      logger.trace(constx.RUNHOOK.MSG.BEGIN, ctxname, routine, operation, entity, JSON.stringify(doc));
      runhookManager.callRunhook(routine, entity, operation, doc).then(function(result) {
        logger.trace(constx.RUNHOOK.MSG.RESULT, ctxname, routine, operation, entity, JSON.stringify(doc), JSON.stringify(result));
        done && done(null, result);
      }).catch(function(error) {
        logger.error(constx.RUNHOOK.MSG.ERROR, ctxname, routine, operation, entity, JSON.stringify(doc), JSON.stringify(error));
        done && done(error, null);
      }).finally(function() {
        logger.trace(constx.RUNHOOK.MSG.END, ctxname, routine, operation, entity, JSON.stringify(doc));
      });
    } else {
      logger.trace(constx.RUNHOOK.MSG.NOOP, ctxname, routine, operation, entity, JSON.stringify(doc));
      done && done(null, {});
    }
  };
  ...
  lodash.forEach(jobQueueEvents, function(event) {
    jobQueue.process(ctxname + '-' + event, jobQueueProcess);
  });
