...
Service.prototype.getIndexSettings = function() {
  ...
  return new Promise(function(resolve, reject) {
    ...
    .end(function(err, res) {
      if (err) {
        logger.trace(' ->> request to index/_settings is error: %s', err);
        reject(err);
      } else {
        logger.trace(' ->> success on getting index/_settings: %s', res.status);
        resolve(res.body);
      }
    });
  });
...
Service.prototype.getIndexMappings = function() {
  ...
  return new Promise(function(resolve, reject) {
    ...
    .end(function(err, res) {
      if (err) {
        logger.trace(' ->> request to index/_mappings is error: %s', err);
        reject(err);
      } else {
        logger.trace(' ->> success on getting index/_mappings: %s', res.status);
        resolve(res.body);
      }
    });
  });
