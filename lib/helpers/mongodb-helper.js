...
Service.prototype.updateDocument = function(entity, criteria, data, options) {
  ...
  var promisee = function (done) {
    self.mongodb[entity].update(criteria, {$set: data}, options, function(err, info) {
      if (err) {
        logger.trace(' ->> Saving %s document with criteria %s is error: %s', entity, JSON.stringify(criteria), err);
        done(err);
      } else {
        logger.trace(' ->> Saving %s document with criteria %s successul: %s', entity, JSON.stringify(criteria), JSON.stringify(info));
        done(null, info);
      }
    });
  };
