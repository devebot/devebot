...
Service.prototype.findOneDocument = function(entity, criteria) {
  ...
  return Promise.promisify(function(callback) {
    self.mongodb[entity].findOne(criteria, function(err, obj) {
      if (err) {
        self.logger.error('<%s> - findOneDocument("%s", "%s") has error: %s', self.getSandboxName(),
            entity, JSON.stringify(criteria), JSON.stringify(err));
      } else {
        self.logger.trace('<%s> - findOneDocument("%s", "%s") result: %s', self.getSandboxName(),
...
Service.prototype.getDocumentById = function(entity, id) {
  ...
  return Promise.promisify(function(callback) {
    ...
    self.mongodb[entity].findOne({
      ...
      if (err) {
        self.logger.error('<%s> - getDocumentById("%s", "%s") has error: %s', self.getSandboxName(), 
            entity, id, JSON.stringify(err));
      } else {
        self.logger.trace('<%s> - getDocumentById("%s", "%s") result: %s', self.getSandboxName(),
...
Service.prototype.getDocumentsByIds = function(entityName, documentIds) {
  var self = this;
  self.logger.trace('<%s> + getDocumentsByIds("%s", "%s")', self.getSandboxName(), 
  ...
  return Promise.promisify(function(callback) {
    ...
    self.mongodb[entityName].find({
      ...
      if (err) {
        self.logger.error('<%s> - getDocumentsByIds("%s", "%s") has error: %s', self.getSandboxName(), 
            entityName, JSON.stringify(documentIds), JSON.stringify(err));
      } else {
        self.logger.trace('<%s> - getDocumentsByIds("%s", "%s") result: %s', self.getSandboxName(), 
...
Service.prototype.insertDocument = function(entity, documents) {
  ...
  return Promise.promisify(function (done) {
    self.mongodb[entity].insert(documents, function(err, result) {
  		if (err) {
  		  self.logger.trace('<%s> - insert documents %s of %s error: %s', self.getSandboxName(),
  		      JSON.stringify(documents), entity, err);
  		} else {
  		  self.logger.trace('<%s> - insert documents %s of %s successful: ', self.getSandboxName(),
...
Service.prototype.updateDocument = function(entity, criteria, data, options) {
  ...
  var promisee = function (done) {
    self.mongodb[entity].update(criteria, {$set: data}, options, function(err, info) {
      if (err) {
        self.logger.trace('<%s> - update %s document: %s with options %s and criteria %s has error: %s', self.getSandboxName(),
            entity, JSON.stringify(data), JSON.stringify(options), JSON.stringify(criteria), err);
      } else {
        self.logger.trace('<%s> - update %s document: %s with options %s and criteria %s successul: %s', self.getSandboxName(),
...
Service.prototype.deleteDocument = function(entityName, criteria) {
  ...
  var promisee = function (done) {
    self.mongodb[entityName].remove(criteria, function(err, result) {
  		if (err) {
  		  self.logger.trace('<%s> - delete %s document with criteria %s has error: %s', self.getSandboxName(),
  		      entityName, JSON.stringify(criteria), err);
  		} else {
  		  self.logger.trace('<%s> - delete %s document with criteria %s result: %s', self.getSandboxName(),
